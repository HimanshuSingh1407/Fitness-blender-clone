...
const Membership_features_array = [
  {
...
let tick_to_yes = <AiOutlineCheckCircle color="rgb(80,188,53)" />;
const MemberShip_Table_Section = () => {
  return (
    <Box px="15px">
      ...
      <TableContainer>
        <Table size="md" fontSize={"18px"}>
          ...
          <Tbody>
            {Membership_features_array.map((item) => (
              <Tr  >
                <Td>{item.feature}</Td>
                <Td fontSize={"25px"}>{tick_to_yes}</Td>
                <Td fontSize={"25px"}>{item.free ? tick_to_yes : ""}</Td>
              </Tr>
            ))}
          </Tbody>
          <Tfoot>
...
export default MemberShip_Table_Section;
