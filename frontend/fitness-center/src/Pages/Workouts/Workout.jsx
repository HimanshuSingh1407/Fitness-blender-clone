...
const Workout = () => {
  ...
  const [strengthData,setStrengthData]=useState([])
   ...
   async function getWorkoutData (){
     let res=await axios.get(`https://backend-server-300e.onrender.com/workouts?limit=4`)
     let data=await res.data;
      setWorkoutData(data)
     }

     async function getPlusWorkoutData (){
      let res=await axios.get(`https://backend-server-300e.onrender.com/workouts?limit=4&page=2`)
      let data=await res.data;
      setPlusData(data)
      }

      async function getBeginnerWorkoutData (){
        let res=await axios.get(`https://backend-server-300e.onrender.com/workouts?limit=4&page=3`)
        let data=await res.data;
      ...
      async function getHILTWorkoutData (){
        let res=await axios.get(`https://backend-server-300e.onrender.com/workouts?limit=4&page=4`)
        let data=await res.data;
        setHILTData(data)
        }

          async function getstrengthWorkoutData (){
            let res=await axios.get(`https://backend-server-300e.onrender.com/workouts?limit=4&page=5`)
            let data=await res.data;
  ...
  return (
    <Box fontFamily={"Helvetica"}>
        <Hide below='md'>
          ...
          <Image h={"600px"} src="https://d18zdz9g6n5za7.cloudfront.net/workouts_programs/masthead-workouts.jpg" w={{sm:"800px",md:"1000px",lg:"1200px",xl:"1600px"}} alt="Screenshot-2022-11-09-123312" border="0"/>
          </Box>
        </Hide>
        <Show below='md'>
          <Box>
            <Image h={{base:"250px",sm:"400px",md:"400px",lg:"500px"}}  w={{base:"500px",sm:"800px",md:"1100",lg:"1200px"}} src="https://d18zdz9g6n5za7.cloudfront.net/workouts_programs/masthead-workouts-xs.jpg" alt="Screenshot-2022-11-09-123312" border="0"/> 
          </Box>
          <Box  display={"grid"} justifyContent={"center"} m={"auto"} h={"200px"} pt={"20px"} fontSize={"11px"} bgColor={"#212432"} w={{base:"450px",sm:"700px",md:"900px"}}>
                 <Text color={"#4296cb"}>ONLINE WORKOUT VIDEOS</Text>
      ...
      <Box bgColor={"#e3ebee"}>
          <Box pb={"90px"} pt={"90px"}>
            <Box margin={"auto"} w={{base:"400px",md:"800px",lg:"900px",xl:"1250px"}}  display={"flex"} alignItems={"center"} justifyContent={"space-between"}>
            <Box fontSize={{base:"18px",md:"32px",lg:"40px"}}>Newest Free Workout Videos</Box>
          ...
          {
            workoutData.map((el)=>{
              return <WorkoutComp key={el._id} image={el.lazyfade_src} text1={el.content_title} text2={el.primary_detail} text3={el.sub_title}/> 
            })
          }
          </Box>
       </Box>
      ...
      <Box bgColor={"#f0f4f6"}>
          <Box pb={"90px"} pt={"90px"}>
            <Box margin={"auto"} w={{base:"400px",md:"800px",lg:"900px",xl:"1250px"}} display={"flex"} alignItems={"center"} justifyContent={"space-between"}>
            <Box fontSize={{base:"18px",md:"32px",lg:"40px"}}>Newest Plus Workout Videos</Box>
      ...
      <Box bgColor={"#e3ebee"}>
          <Box pb={"90px"} pt={"90px"}>
            <Box margin={"auto"} w={{base:"400px",md:"800px",lg:"900px",xl:"1250px"}} display={"flex"} alignItems={"center"} justifyContent={"space-between"}>
            <Box fontSize={{base:"18px",md:"32px",lg:"40px"}}>Beginner Workouts</Box>
      ...
      <Box bgColor={"#f0f4f6"}>
          <Box pb={"90px"} pt={"90px"}>
            <Box margin={"auto"} w={{base:"400px",md:"800px",lg:"900px",xl:"1250px"}} display={"flex"} alignItems={"center"} justifyContent={"space-between"}>
            <Box fontSize={{base:"18px",md:"32px",lg:"40px"}}>HIIT Workouts</Box>
      ...
      <Box bgColor={"#e3ebee"}>
          <Box pb={"90px"} pt={"90px"}>
            <Box margin={"auto"} w={{base:"400px",md:"800px",lg:"900px",xl:"1250px"}} display={"flex"} alignItems={"center"} justifyContent={"space-between"}>
            <Box fontSize={{base:"18px",md:"32px",lg:"40px"}}>Strength Workouts</Box>
